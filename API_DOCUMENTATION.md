...
5. After notification is sent, `notifiedAt` is updated (reminder is **not** advanced)
6. Recurring todos (`schedule.repeat` = `daily`/`weekly`/`monthly`): `completed` means done for the **current period** only. Rollover on read/scheduler sets `reminder` to today's slot and clears `completed` when the period ends. `toggleComplete` logs the occurrence and points `reminder` at the next slot. `lastCompletedAt` + `/todos/{id}/completions` are history.
